...
export class QuizComponent implements OnInit {
  ...
  ngOnInit() {
    this.count=false;
    const items1=this.firestore.collection('Quiz').snapshotChanges();
    items1.forEach(p=>{
    const l=p.map((value,index)=>{
    if(index===0){
    const hj= value.payload.doc.get('ids');
    console.log(hj);
    const g=this.firestore.collection('Questions data').doc(hj).snapshotChanges();
    g.subscribe(gy=>{
...
goto()
{
  ...
  //localStorage.setItem('user selected',this.s);
  ...
  ||(this.s===this.temp19)||(this.s===this.temp20)||(this.s===this.temp21))
  {
    ...
    const dat=('you selected') + this.s +('correct answer');
    this.crudservice.id2(dat).then(res=>{
      console.log(res);
    });
  }
  else
  {console.log('wrong answer');
  const data=('you selected')+this.s +('wrong answer');
  this.crudservice.id2(data).then(res=>{
    console.log(res);
  });
  }
}
